...
public abstract class BaseEventAction extends BaseAction {
    ...
    protected void publishIVREvent(IVREvent name, String externalId, Map<String, String> requestParams, IVRRequest.CallDirection callDirection, String callerId, String inputData, String responseXML) {

        EventDataBuilder builder = new EventDataBuilder(name.toString(), externalId, requestParams, DateUtil.now());
    ...
    public String handleInternal(IVRRequest ivrRequest, HttpServletRequest request, HttpServletResponse response) {
        ...
        publishIVREvent(ivrRequest.callEvent(), patientId, request.getParameterMap(), ivrRequest.getCallDirection(), ivrRequest.getCallerId(), ivrRequest.getData(), responseXML);
